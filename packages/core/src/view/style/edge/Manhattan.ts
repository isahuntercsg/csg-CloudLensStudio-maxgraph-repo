...
// fallback route
const MANHATTAN_MAXIMUM_LOOPS = 2000;

// Possible starting directions from an element
const MANHATTAN_START_DIRECTIONS: DIRECTION[] = [
  DIRECTION.NORTH,
  DIRECTION.EAST,
  DIRECTION.SOUTH,
  DIRECTION.WEST,
...
const MANHATTAN_END_DIRECTIONS: DIRECTION[] = [
  DIRECTION.NORTH,
  DIRECTION.EAST,
  DIRECTION.SOUTH,
  DIRECTION.WEST,
...
const MANHATTAN_MAX_ALLOWED_DIRECTION_CHANGE = 90;

const MANHATTAN_PADDING_BOX = new Geometry(
  -MANHATTAN_STEP,
  -MANHATTAN_STEP,
  MANHATTAN_STEP * 2,
  MANHATTAN_STEP * 2
...
export function ManhattanConnector(
  ...
  const mStep = MANHATTAN_STEP;

  const config = {
    // Padding applied on the element bounding boxes
    paddingBox: MANHATTAN_PADDING_BOX,
    ...
    penaltiesGenerator: (angle: number) => {
      if (angle == 45 || angle == 90 || angle == 180) return MANHATTAN_STEP / 2;
      return 0;
    },
  ...
  function alignPointToCell(
    ...
    const onlyHorizontalDirections = isSourceCell
      ? MANHATTAN_START_DIRECTIONS.every(
          (d) => d != DIRECTION.NORTH && d != DIRECTION.SOUTH
        )
      : MANHATTAN_END_DIRECTIONS.every(
          (d) => d != DIRECTION.NORTH && d != DIRECTION.SOUTH
        );
    ...
    const onlyVerticalDirections = isSourceCell
      ...
      : MANHATTAN_END_DIRECTIONS.every((d) => d != DIRECTION.WEST && d != DIRECTION.EAST);
  ...
  function findRoute(
    ...
    const step = MANHATTAN_STEP;
    const startPoints = getRectPoints(start, MANHATTAN_START_DIRECTIONS, opt).filter(
      (p) => obstacleMap.isPointAccessible(p)
    );

    const startCenter = snapPointToGrid(getRectangleCenter(start), step);
    const endPoints = getRectPoints(end, MANHATTAN_END_DIRECTIONS, opt).filter((p) =>
      obstacleMap.isPointAccessible(p)
    );
    ...
    if (startPoints.length > 0 && endPoints.length > 0) {
      ...
      let loopsRemain = MANHATTAN_MAXIMUM_LOOPS;
      ...
      while (!openSet.isEmpty() && loopsRemain > 0) {
        ...
        for (let i = 0; i < opt.directions.length; i++) {
          ...
          if (
            ...
            directionChangedAngle > MANHATTAN_MAX_ALLOWED_DIRECTION_CHANGE
          ) {
            continue;
          }
          ...
          const costFromStart =
            currentCost + direction.cost + opt.penaltiesGenerator(directionChangedAngle);
...
function getRoutePattern(
  ...
  let result: number[] | null = routePatterns[sourceIndex - 1][targetIndex - 1];
